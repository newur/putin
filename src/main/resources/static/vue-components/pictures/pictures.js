Vue.component("pictures", {
    ...
    created: function() {
        ...
        this.timer = setInterval(this.showNext, 10000)
    },
    mounted: function(){
        this.showFirst();
    },
    methods: {
        showFirst: function() {
            if (this.unshownImages.length > 1) {
                //load bottom picture
                var randomInt = Math.floor(Math.random() * this.unshownImages.length);
                var imageBottom = document.getElementById('bottom');
                imageBottom.src = this.unshownImages[randomInt];
                this.unshownImages.splice(randomInt,1);

                //load top picture
                var randomInt = Math.floor(Math.random() * this.unshownImages.length);
                var imageTop = document.getElementById('top');
                imageTop.src = this.unshownImages[randomInt];
                this.unshownImages.splice(randomInt,1);
                this.active = 'top';
            }
        },
        showNext: function() {
            ...
            if(this.active == 'bottom'){
                $( "#top" ).fadeIn( "2000", function() {
                    var randomInt = Math.floor(Math.random() * self.unshownImages.length);
                    document.getElementById('bottom').src = self.unshownImages[randomInt];
                    self.unshownImages.splice(randomInt,1);
                    self.active = 'top';
                });
            }
            else {
                $( "#top" ).fadeOut( "2000", function() {
                    var randomInt = Math.floor(Math.random() * self.unshownImages.length);
                    document.getElementById('top').src = self.unshownImages[randomInt];
                    self.unshownImages.splice(randomInt,1);
                    self.active = 'bottom';
                });
            }
